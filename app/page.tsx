  ...
  export default function UserPage() {
        ...
        try {
          ...
          try {
            ...
            if (Array.isArray(raw)) {
              ...
              console.warn(
                "Format /api/games tidak sesuai (bukan array), status:",
                gamesRes.status
              );
            }
          } catch (e) {
            console.error("Gagal parse JSON /api/games, status:", gamesRes.status);
          }
          setGames(gamesData);
          ...
          //  PS ROOMS
          // =========================
          let roomsData: PsStatus[] = [];
          try {
            const raw = await psRes.json();
            console.log("RAW /api/ps:", raw);
            ...
            if (Array.isArray(raw)) {
              ...
              console.warn(
                "Format /api/ps tidak sesuai (bukan array), status:",
                psRes.status
              );
            }
          } catch (e) {
            console.error("Gagal parse JSON /api/ps, status:", psRes.status);
          }
          setRooms(roomsData);
        } catch (err) {
          console.error("Error fetch data (network/server error):", err);
          setGames([]);
          setRooms([]);
        } finally {
          setLoading(false);
    ...
    return (
      ...
      shadow-[0_0_14px_rgba(74,222,128,0.45)]
      backdrop-blur-xl
      transition
    "
        >
    ...
    "
      ...
      "
        ...
        hover:bg-sky-500/25 hover:border-sky-200/40
      ...
      "
          >
            <PhoneCall className="w-4 h-4 sm:w-5 sm:h-5 text-sky-300 drop-shadow-[0_0_6px_rgba(56,189,248,0.8)]" />
            Booking Sekarang
          </Link>
        </div>
